...
$(document).ready(function() {
	...
	$('table#ahisCrudTable thead tr th').each(function(index){
	...
	oTable = $('#ahisCrudTable').dataTable({
		"bJQueryUI": true,
	    ...
	    "oTableTools": {
	    	"aButtons": aButtons,
	        "sSwfPath": base_url+"assets/ahis_crud/themes/datatables/extras/TableTools/media/swf/copy_csv_xls_pdf.swf"
	    }
	});

	$("#ahisCrudTable tfoot input").keyup( function () {
		oTable.fnFilter( this.value, $("#ahisCrudTable tfoot input").index(this) );
		
		if(use_storage)
		{
			var search_values_array = [];
			
			$("#ahisCrudTable tfoot tr th").each(function(index,value){
	...
	if( search_values !== null)
	{
		$.each($.parseJSON(search_values),function(num,val){
			if(val !== '')
			{
				$("#ahisCrudTable tfoot tr th:eq("+num+")").children(':first').val(val);
			}
		});
	}
	...
	$('.clear-filtering').click(function(){
		...
		$("#ahisCrudTable tfoot tr th input").val("");
	});
