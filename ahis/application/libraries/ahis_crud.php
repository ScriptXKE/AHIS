 ...
 * LICENSE
 *
 * ahis CRUD is released with dual licensing, using the GPL v3 (license-gpl3.txt) and the MIT license (license-mit.txt).
 ...
 * @license    	https://github.com/scoumbourdis/ahis-crud/blob/master/license-ahis-crud.txt
 * @version    	1.3
 ...
 * @author     	John Skoumbourdis <scoumbourdisj@gmail.com>
 * @license     https://github.com/scoumbourdis/ahis-crud/blob/master/license-ahis-crud.txt  
 * @link		http://www.ahiscrud.com/documentation
 */
class ahis_CRUD_Field_Types
{	
	/**	 
 ...
 * @link		http://www.ahiscrud.com/documentation
 */
class ahis_CRUD_Model_Driver extends ahis_CRUD_Field_Types
{
	/**
	 * @var ahis_CRUD_Model
	 */
	...
	{
		$ci = &get_instance();
		$ci->load->model('ahis_CRUD_Model');
		
		$this->basic_model = $ci->ahis_CRUD_Model;
	}
	...
	protected function form_validation()
	{
		...
		{
			$this->form_validation = new ahis_CRUD_Form_validation();
			$ci = &get_instance();
 ...
 * @version    	1.3
 */
class ahis_CRUD_Layout extends ahis_CRUD_Model_Driver
{
	...
	{
		$this->set_js($this->default_javascript_path.'/'.ahis_CRUD::JQUERY);
	...
	{		
		...
		if($this->unset_jquery)
			unset($js_files[sha1($this->default_javascript_path.'/'.ahis_CRUD::JQUERY)]);
		
		if($this->unset_jquery_ui)
		{
			unset($css_files[sha1($this->default_css_path.'/ui/simple/'.ahis_CRUD::JQUERY_UI_CSS)]);
			unset($js_files[sha1($this->default_javascript_path.'/jquery_plugins/ui/'.ahis_CRUD::JQUERY_UI_JS)]);
		}
	...
	{
		$this->set_css($this->default_css_path.'/ui/simple/'.ahis_CRUD::JQUERY_UI_CSS);
		...
		$this->set_js($this->default_javascript_path.'/jquery_plugins/ui/'.ahis_CRUD::JQUERY_UI_JS);
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input .= "<div id='uploader_$unique' rel='$unique' class='ahis-crud-uploader' style='$uploader_display_none'></div>";
 ...
 * @version    	1.3
 */
class ahis_CRUD_States extends ahis_CRUD_Layout
{
 ...
 * @link		http://www.ahiscrud.com/documentation
 */
class ahis_CRUD extends ahis_CRUD_States
{
	/**
	 * ahis CRUD version
	 * 
	...
	protected $default_language_path				= 'assets/ahis_crud/languages';
	protected $default_config_path					= 'assets/ahis_crud/config';
	protected $default_assets_path					= 'assets/ahis_crud';
...
//@todo ui_day, ui_month, ui_year has to be lang strings
	...
	{
		$ci = &get_instance();
		$ci->load->config('ahis_crud');
		
		$this->config = (object)array();
		
		/** Initialize all the config variables into this object */
		$this->config->default_language 	= $ci->config->item('ahis_crud_default_language');
		$this->config->date_format 			= $ci->config->item('ahis_crud_date_format');
		$this->config->default_per_page		= $ci->config->item('ahis_crud_default_per_page');
		$this->config->file_upload_allow_file_types	= $ci->config->item('ahis_crud_file_upload_allow_file_types');
		$this->config->file_upload_max_file_size	= $ci->config->item('ahis_crud_file_upload_max_file_size');
		$this->config->default_text_editor	= $ci->config->item('ahis_crud_default_text_editor');
		$this->config->text_editor_type		= $ci->config->item('ahis_crud_text_editor_type');
		$this->config->character_limiter	= $ci->config->item('ahis_crud_character_limiter');
	 ...
	 * Without this function nothing works. Here is the core of ahis CRUD project.
	 * 
	 * @access	public
	 */
...
if(defined('CI_VERSION'))
{
	...
	class ahis_CRUD_Form_validation extends CI_Form_validation{
