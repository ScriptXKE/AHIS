...
/**
 * This class contains methods for the settlements
...
class Settlement extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['view'] = 'settlement/index';

		$paginate['base_url'] = base_url().'/settlement/index/';
		$paginate['total_rows'] = $this->settlement_model->count('district');
	...
	public function detail($settlement_ID)
	{
		$data = array(); // this will prevent errors in case there is no data in the array; so we declare it and leave it empty

		if ($query = $this->settlement_model->get_settlement_detail($settlement_ID))
		{
			$data['view'] = 'settlement/detail';

			$this->db->select('country.name country_name, zone.name zone_name, region.name region_name, district.name district_name, district.id district_id, settlement.name settlement_name,  settlement.id settlement_id');
			$this->db->from('country');
			$this->db->where('district.id', $settlement_ID);
			$this->db->join('zone', 'zone.country_id = country.id');
			$this->db->join('region', 'region.zone_id = zone.id');
			$this->db->join('district', 'district.region_id = region.id');
			$this->db->join('settlement', 'settlement.district_id = district.id');
